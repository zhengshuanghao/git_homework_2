...
try:
    ...
    print("tkinterdnd2未安装，拖拽功能不可用。可以使用：pip install tkinterdnd2")
    DND_AVAILABLE = False
...
class WatermarkApp:
    ...
    def create_file_panel(self, parent):
        ...
        # 提示标签
        tip_label = ttk.Label(import_frame, text="💡 提示：也可以复制图片路径使用", 
                              font=('Arial', 8), foreground='gray')
        tip_label.pack(fill=tk.X, pady=(5, 5))
        ...
        # 添加快捷导入功能
        self.setup_quick_import(file_frame)
    ...
    def create_preview_panel(self, parent):
        ...
        self.canvas.create_text(400, 300, text=hint_text, 
                               font=('Arial', 16), fill='gray', tags="hint", justify=tk.CENTER)
    ...
    def setup_quick_import(self, target_frame):
        """设置快捷导入功能"""
        # 添加从剪贴板导入路径的功能
        quick_frame = ttk.Frame(target_frame)
        quick_frame.pack(fill=tk.X, pady=(5, 0))
        
        ttk.Button(quick_frame, text="从剪贴板导入路径", 
                  command=self.import_from_clipboard).pack(fill=tk.X, pady=(0, 5))
        
        # 提示信息
        tip_label = ttk.Label(quick_frame, 
                             text="💡 复制图片文件路径到剪贴板，然后点击上方按钮", 
    ...
    def import_from_clipboard(self):
        ...
        try:
            ...
            # 处理可能的多个路径（按行分割）
            paths = [path.strip() for path in clipboard_content.split('\n') if path.strip()]
            ...
            if image_files:
                ...
                self.status_var.set(f"从剪贴板导入了 {len(image_files)} 个图片文件")
            else:
                messagebox.showinfo("提示", "剪贴板中没有找到有效的图片文件路径\n\n"
                                           "请复制图片文件的完整路径到剪贴板\n"
            ...
            messagebox.showerror("错误", f"从剪贴板导入时出错：{str(e)}")
    ...
    def setup_drag_drop(self):
        ...
        # 绑定拖拽事件
        self.canvas.dnd_bind('<<DropEnter>>', self.on_drag_enter)
        self.canvas.dnd_bind('<<DropLeave>>', self.on_drag_leave) 
    ...
    def on_drop(self, event):
        ...
        if valid_files:
            ...
            # 自动选择并预览第一个新添加的图片
            if len(self.selected_images) == len(valid_files):
                # 如果之前没有图片，选择第一个
                self.current_image_index = 0
                self.file_listbox.selection_set(0)
            else:
                # 选择第一个新添加的图片
                self.current_image_index = len(self.selected_images) - len(valid_files)
            ...
            if len(valid_files) == 1:
                self.status_var.set(f"成功导入图片: {os.path.basename(valid_files[0])}")
            else:
                self.status_var.set(f"成功导入 {len(valid_files)} 张图片")
        
        # 如果有无效文件，显示警告
        if invalid_files:
            invalid_names = [os.path.basename(f) for f in invalid_files[:3]]
