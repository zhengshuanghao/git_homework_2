...
class WatermarkManager:
    ...
    def create_text_watermark(self, image: Image.Image) -> Image.Image:
        ...
        print(f"调试信息: 粗体={self.font_bold}, 斜体={self.font_italic}, 字体={self.font_family}")
        ...
        # 模拟粗体效果 - 如果字体文件不支持粗体，通过多次绘制来模拟
        if self.font_bold:
            # 检查是否成功加载了粗体字体文件
            font_path = self._get_styled_font_path(self.font_family, self._has_chinese_chars(self.watermark_text))
            if not font_path or not os.path.exists(font_path):
                # 字体文件不存在，使用多重绘制模拟粗体
                bold_offsets = [(0, 0), (1, 0), (0, 1), (1, 1), (2, 0), (0, 2)]
                for offset_x, offset_y in bold_offsets:
                    draw.text((base_x + offset_x, base_y + offset_y), self.watermark_text, font=font, fill=main_color)
            else:
                # 有粗体字体文件，正常绘制
        ...
        # 对于斜体效果，我们需要在后续的变换中处理
        
        # 应用斜体和旋转变换
        needs_transform = self.font_italic or self.rotation_angle != 0
        
        if needs_transform:
            # 应用斜体变换
            if self.font_italic:
                # 使用仿射变换创建斜体效果
                # 斜体变换矩阵 [1, shear, 0, 1, 0, 0] 其中shear为倾斜系数
                shear = 0.2  # 倾斜度
                width, height = text_layer.size
                # 计算变换后的新尺寸
                new_width = int(width + height * abs(shear))
                new_height = height
                
                # 创建新的图层用于斜体变换
                italic_layer = Image.new('RGBA', (new_width, new_height), (0, 0, 0, 0))
                
                # 应用仿射变换
                transform_matrix = (1, shear, -height * shear if shear > 0 else 0, 0, 1, 0)
                italic_layer = text_layer.transform((new_width, new_height), Image.AFFINE, transform_matrix, 
                                                  resample=Image.BICUBIC, fillcolor=(0, 0, 0, 0))
                text_layer = italic_layer
